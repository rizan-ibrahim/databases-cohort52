...
async function createEpisodeExercise(client) {
  ...
  const result = await episodes.insertOne(episode);
...
async function findEpisodesExercises(client) {
  ...
  const db = client.db("databaseWeek3");
  const episodes = db.collection("bob_ross_episodes");
  const result1 = await episodes.findOne({ episode: "S02E02" });

  console.log(`The title of episode 2 in season 2 is ${result1?.title}`);

  // Find the season and episode number of the episode called "BLACK RIVER" [Should be: S02E06]

  const result2 = await episodes.findOne({ title: "BLACK RIVER" });
  console.log(
  ...
  const result3 = await episodes
    .find({ elements: "CLIFF" }, { projection: { title: 1, _id: 0 } })
    .toArray();
  const titles3 = result3.map((e) => e.title);

  console.log(
    `The episodes that Bob Ross painted a CLIFF are ${titles3.join(", ")}}`
  );

  // Find all of the episode titles where Bob Ross painted a CLIFF and a LIGHTHOUSE [Should be: NIGHT LIGHT]

  const result4 = await episodes
    .find(
      { elements: { $all: ["CLIFF", "LIGHTHOUSE"] } },
      { projection: { title: 1, _id: 0 } }
    )
    .toArray();
  const titles4 = result4.map((e) => e.title);

  console.log(
    `The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE are ${titles4.join(
      ", "
    )}`
  );
}
...
async function updateEpisodeExercises(client) {
  ...
  const result1 = await episodes.updateOne(
    { episode: "S30E13" },
  ...
  const result2 = await episodes.updateMany(
    { elements: "BUSHES" },
    { $set: { "elements.$": "BUSH" } }
  );
...
async function deleteEpisodeExercise(client) {
  ...
  const result = await episodes.deleteOne({ episode: "S31E14" });
